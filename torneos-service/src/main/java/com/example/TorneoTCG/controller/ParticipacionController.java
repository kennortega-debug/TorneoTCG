...
public class ParticipacionController {
    ...
    @GetMapping
    public ResponseEntity<List<ParticipacionDTO>> todas() {
        List<ParticipacionDTO> lista = participacionService.obtenerTodos();
    ...
    @PostMapping
    public ResponseEntity<?> registrar(@Valid @RequestBody Participacion participacion) {
        try {
            ParticipacionDTO dto = participacionService.guardar(participacion);
    ...
    @PutMapping("/{id}")
    public ResponseEntity<?> actualizar(@PathVariable Long id, @Valid @RequestBody Participacion participacion) {
    try {
        ParticipacionDTO dto = participacionService.actualizar(id, participacion);
        return new ResponseEntity<>(dto, HttpStatus.OK);
    } catch (RuntimeException e) {
        return new ResponseEntity<>(e.getMessage(), HttpStatus.NOT_FOUND);
    } catch (Exception e) {
    ...
    String resultado = participacionService.eliminar(id);
